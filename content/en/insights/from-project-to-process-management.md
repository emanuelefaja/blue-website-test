...
1. Conditional automations (where you can have multiple criteria that must be true before something happens)
2. Trying to solve the infinite loop problem (where right now an automation cannot trigger another automation because that could cause an infinite loop). In reality, we would love for that to be possible and we just detect infinite loops and make sure that you cannot create them. 
